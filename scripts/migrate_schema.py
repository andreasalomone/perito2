    ...
    with app.app_context():
        ...
        try:
            ...
            with db.engine.connect() as conn:
                conn.execute(text("ALTER TABLE report_log ADD COLUMN IF NOT EXISTS llm_raw_response TEXT;"))
                conn.execute(text("ALTER TABLE report_log ADD COLUMN IF NOT EXISTS final_report_text TEXT;"))
                conn.commit()
            ...
            with db.engine.connect() as conn:
                ...
                conn.execute(text("ALTER TABLE document_log ADD COLUMN IF NOT EXISTS extraction_status VARCHAR(50);"))
                
                conn.execute(text("ALTER TABLE document_log ADD COLUMN IF NOT EXISTS extracted_content_length INTEGER DEFAULT 0;"))
                conn.execute(text("ALTER TABLE document_log ADD COLUMN IF NOT EXISTS error_message TEXT;"))
                conn.execute(text("ALTER TABLE document_log ADD COLUMN IF NOT EXISTS file_type VARCHAR(50);"))
                conn.commit()
