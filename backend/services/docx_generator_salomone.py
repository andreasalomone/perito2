...
def create_styled_docx(plain_text_report_content: str) -> io.BytesIO:
    ...
    if not os.path.exists(
        ...
        logo_path = "assets/logos/salomone-logo.png"
    ...
    # --- Footer con layout Semplificato: Numero Pagina in alto a dx, Testo statico centrato ---
    footer_static_text_line1 = "Salomone & ASSOCIATI Srl – Via Ippolito d’Aste 3/12 – 16121 Genova (Italia) – Tel +39 010 0980210"
    footer_static_text_line2 = "Codice Fiscale e P.IVA 02286960998 – REA 474817 – PEC salomoneassociatisrl@legalmail.it"
    ...
    # 2. Paragrafo per il testo statico
    p_text = footer.add_paragraph()
    p_text.alignment = WD_ALIGN_PARAGRAPH.CENTER

    # Rimuovi qualsiasi indentazione ereditata
    fmt_text = p_text.paragraph_format
    fmt_text.first_line_indent = Pt(0)
    fmt_text.space_before = Pt(0)
    fmt_text.space_after = Pt(0)
    fmt_text.line_spacing = 1.0

    # Popola il paragrafo con il testo statico, formattando "Salomone & ASSOCIATI Srl"
    parts1 = footer_static_text_line1.split("Salomone & ASSOCIATI Srl", 1)
    if len(parts1) == 2:
        if parts1[0]:
            p_text.add_run(parts1[0])
        run_company1 = p_text.add_run("Salomone & ASSOCIATI Srl")
