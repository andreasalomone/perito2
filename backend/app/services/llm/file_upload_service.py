...
@retry(
    ...
    wait=wait_exponential(
        multiplier=1, min=settings.LLM_API_RETRY_WAIT_SECONDS, max=10
    ),
    retry=retry_if_exception_type(RETRIABLE_EXCEPTIONS),
    reraise=True,
)
def _execute_blocking_upload(
    client: genai.Client, path: str, config: types.UploadFileConfig
) -> types.File:
...
async def upload_single_file(
    client: genai.Client, candidate: UploadCandidate, semaphore: asyncio.Semaphore
) -> FileOperationResult:
    """
    ...
    async with semaphore:
        try:
            ...
            if not os.path.exists(target_path) and candidate.gcs_uri:
                logger.debug(
                    f"Local file missing for {safe_name}. Downloading from {candidate.gcs_uri}..."
                )
            ...
            uploaded_file = await asyncio.to_thread(
                _execute_blocking_upload, client, target_path, upload_config
            )

            logger.info(
                f"Upload success: {candidate.display_name} -> {uploaded_file.uri}"
            )
            ...
            return FileOperationResult(
                file_name=candidate.display_name, success=False, error_message=error_msg
            )
        finally:
...
async def upload_vision_files_batch(
    client: genai.Client, files: Sequence[UploadCandidate]
    ...
    if len(valid_candidates) < len(files):
        logger.warning(
            f"Skipped {len(files) - len(valid_candidates)} invalid or non-vision files."
        )
...
async def delete_single_file(
    client: genai.Client, file_name: str, semaphore: asyncio.Semaphore
) -> bool:
