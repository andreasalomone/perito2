...
# -----------------------------------------------------------------------------
# SYNCHRONOUS WRAPPERS (For Cloud Tasks / 'def' Endpoints)
# -----------------------------------------------------------------------------

def process_case_logic_sync(case_id: str, organization_id: str, db: Session):
    """
    Synchronous version of process_case_logic.
    """
    # 1. Setup - Get Case with locking
    case = db.query(Case).filter(Case.id == case_id).with_for_update().first()
    if not case:
        logger.error(f"Case {case_id} not found in DB")
        return
    ...
    for doc in documents:
        ...
        if settings.RUN_LOCALLY:
            logger.info(f"Local mode: Processing document {doc.id} inline (SYNC)")
            case_service.process_document_extraction_sync(doc.id, organization_id, db)
        else:
            case_service.trigger_extraction_task(doc.id, organization_id)
    ...
    if all_processed:
        logger.info(f"All documents for case {case_id} already processed. Triggering generation immediately.")
        trigger_generation_task_sync(case_id, organization_id)


def trigger_generation_task_sync(case_id: str, organization_id: str):
    """
    Synchronous version of trigger_generation_task.
    """
    if settings.RUN_LOCALLY:
        logger.info(f"Running generation locally for case {case_id}")
        # Run in background? No, just run sync for local dev simplicity or use a thread.
        # But here we just want to call the logic.
        # We need a fresh DB session if we are not passed one, but this is usually called
        # from a context where we might not have one or want to detach.
        # For simplicity in local mode, let's just run it.
        # But wait, run_generation_task creates its own session.
        run_generation_task_sync(case_id, organization_id)
        return
...
def run_generation_task_sync(case_id: str, organization_id: str):
    """
    Wrapper for background execution (Sync).
    """
    db = SessionLocal()
    try:
        db.execute(
        ...
        generate_report_logic_sync(case_id, organization_id, db)
    finally:
        db.close()

def generate_report_logic_sync(case_id: str, organization_id: str, db: Session):
    """
    Synchronous version of generate_report_logic.
    """
    # 1. Fetch case
    case = db.query(Case).filter(Case.id == case_id).with_for_update().first()
    if not case:
    ...
    pending_docs = [d for d in all_docs if d.ai_status not in ["processed", "error"]]
    if pending_docs:
        logger.info(f"Case {case_id} has pending documents. Skipping generation.")
        return
    
    has_completed_docs = any(d.ai_status == "processed" for d in all_docs)
    if not has_completed_docs:
        logger.error(f"Cannot generate report for case {case_id}: No successfully processed documents.")
        case.status = "error"
        db.commit()
        raise ValueError("No valid documents to generate report from")

    processed_count = sum(1 for d in all_docs if d.ai_status == "processed")
    logger.info(f"Starting generation for case {case_id} with {processed_count} docs.")
    ...
    try:
                ...
                for item in data:
                    if item.get("type") == "vision":
                        ...
                        if target_gcs_path:
                            try:
                                # SYNC DOWNLOAD
                                download_file_to_temp(target_gcs_path, local_path)
                                item["path"] = local_path
                            except Exception as e:
                                logger.error(f"Failed to re-download {target_gcs_path}: {e}")
                                item["type"] = "error"
        ...
        report_result = asyncio.run(gemini_generator.generate(
            processed_files=processed_files_models
        ))
        report_text = report_result.content
        token_usage = report_result.usage.model_dump()
