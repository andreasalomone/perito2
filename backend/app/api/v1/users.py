...
def invite_user(
    ...
    existing_invite = db.scalar(
        select(AllowedEmail).where(AllowedEmail.email == request.email)
    )
    if existing_invite:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
    ...
    try:
        ...
        logger.info(
            f"Invite created: {request.email} for Org {admin_user.organization_id} by {uid}"
        )
...
def update_my_profile(
    ...
    user = db.scalar(
        select(User).options(joinedload(User.organization)).where(User.id == uid)
    )
    if not user:
        raise HTTPException(
