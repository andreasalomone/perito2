...
def verify_brevo_signature(payload_body: bytes, signature: str) -> bool:
    ...
    expected = hmac.new(
        settings.BREVO_WEBHOOK_SECRET.encode("utf-8"), payload_body, hashlib.sha256
    ).hexdigest()
...
async def brevo_inbound_webhook(
    ...
    logger.info(
        f"Received Brevo webhook: message_id={message_id} from={email_item.sender_email}"
    )
    ...
    return WebhookAcceptedResponse(status="accepted", message_id=message_id)


@router.get("/brevo-inbound/health", summary="Webhook Health Check")
