...
class OutboxMessage(Base):
    ...
    topic: Mapped[str] = mapped_column(String(50), nullable=False)  # e.g., "generate_report"
    payload: Mapped[dict[str, Any]] = mapped_column(JSONB, nullable=False)  # The data needed for the task
    status: Mapped[str] = mapped_column(String(20), default="PENDING", nullable=False)  # PENDING, PROCESSED, FAILED
    organization_id: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, nullable=True)  # For tenant isolation
